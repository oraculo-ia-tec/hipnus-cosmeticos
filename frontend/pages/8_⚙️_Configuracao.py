"""
8_⚙️_Configuracao.py — HIPNUS COSMÉTICOS
==========================================
...
Permite:
  ...
  - Alterar senha
  - (Futuro) Preferências de notificação
...
# Carrega dados do banco (se parceiro) ou usa sessão (se demo/seed)
parceiro_db = None
if email_logado and "@" in email_logado:
    parceiro_db = buscar_por_email(email_logado)

# Dados correntes (banco > sessão)
...
avatar_atual = parceiro_db.get("avatar_b64") if parceiro_db else st.session_state.get("avatar_b64")
...
with tab_perfil:
    ...
    # Card de identidade (preview)
    avatar_preview = avatar_atual or ""
    if avatar_preview:
        img_tag = f'<img src="{avatar_preview}" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid #7c3aed;"/>'
    else:
    ...
    if perfil_logado in ("demo",):
        ...
        if salvar:
            if not email_logado or "@" not in email_logado:
                st.warning("⚠️ Perfil seed/demo: alterações não persistem no banco.")
                # Atualiza apenas a sessão
                st.session_state["nome"]         = novo_nome
                st.session_state["display_name"] = novo_username or novo_nome
                st.success("✅ Sessão atualizada (temporário).")
            else:
...
with tab_foto:
    ...
    with col_up:
        ...
        st.caption("JPG, PNG ou WEBP · Máximo 2 MB · Será exibida no sidebar e futuramente no chat da 🤖 IA Consultora")
        ...
        if novo_avatar:
            if novo_avatar.size > 2 * 1024 * 1024:
                ...
                btn_salvar_foto = st.button("💾 Salvar foto", type="primary", use_container_width=True)
                if btn_salvar_foto:
                    ok_s, msg_s = atualizar_perfil(email_logado, avatar_b64=novo_b64) if email_logado and "@" in email_logado else (True, "")
    ...
    if avatar_atual:
        components.divider()
        if st.button("🗑️ Remover foto de perfil", use_container_width=False):
            ok_r, msg_r = atualizar_perfil(email_logado, avatar_b64="") if email_logado and "@" in email_logado else (True, "")
...
with tab_senha:
    ...
    is_seed = perfil_logado in ("super_admin", "admin", "b2b", "b2c") and not parceiro_db
    if is_seed or perfil_logado == "demo":
