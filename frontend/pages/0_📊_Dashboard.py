...
CORREÇÃO: st.html() não acumula tags abertas; bordas aplicadas via CSS em
          containers nativos do Streamlit (border via st.container + CSS injection).
"""
...
# ── CSS global: cartões com borda via classe .grafico-card ────────────────────
st.markdown("""
<style>
/* Cartão de gráfico com borda visível */
.grafico-card {
    background: #faf9ff;
    border: 2.5px solid #e5e0f5;
    border-radius: 14px;
...
.section-sep {
    border: none;
    border-top: 2.5px solid #e5e0f5;
    margin: 24px 0 18px;
    border-radius: 2px;
}
/* Força container Streamlit a renderizar HTML inline */
...
def _fig_to_html(fig) -> str:
    """Converte figura matplotlib para tag <img> base64 inline."""
    buf = io.BytesIO()
...
def _card(title: str, img_html: str) -> str:
    """Monta HTML completo do cartão com borda + título + imagem."""
    return (
        f'<div class="grafico-card">'
        f'<span class="grafico-card-title">{title}</span>'
        f'{img_html}'
        f'</div>'
    )
...
def _resolve_api_key() -> tuple[str, str]:
    import os
    for src, fn in [
        ("st.secrets raiz",    lambda: st.secrets.get("ASAAS_API_KEY", "")),
        ("st.secrets [asaas]", lambda: st.secrets.get("asaas", {}).get("ASAAS_API_KEY", "")),
        ("st.secrets [hipnus]",lambda: st.secrets.get("hipnus", {}).get("ASAAS_API_KEY", "")),
    ]:
        try:
            v = fn()
    ...
    try:
        ...
        keys = []
    return "", f"ASAAS_API_KEY não encontrada. Chaves em st.secrets: {keys}"
...
<div style="background:linear-gradient(135deg,rgba(15,10,30,.85),rgba(30,15,55,.75));
  ...
  padding:16px 22px 12px;margin-bottom:16px;backdrop-filter:blur(8px);">
  <span style="font-family:'Inter',sans-serif;font-size:.63rem;font-weight:700;
...
if asaas_error:
    is_missing = "não encontrada" in asaas_error or "nao encontrada" in asaas_error
    if is_missing:
        with st.expander("⚠️ ASAAS_API_KEY não encontrada", expanded=True):
            st.markdown("""
...
```
            """)
            st.caption(f"Detalhe técnico: {asaas_error}")
    else:
        st.warning(f"Asaas API retornou erro: {asaas_error}")
...
p4.metric("📨 Total convites",  n_invites_total)
...
if not df_pay.empty:
    ...
    _method_map = {"PIX": "PIX", "BOLETO": "Boleto", "CREDIT_CARD": "Cartão"}
    ...
    df_pay["metodo_pt"] = df_pay["billingType"].map(_method_map).fillna(
        df_pay.get("billingType", pd.Series(dtype=str)).fillna("Outro"))
...
# ── Linha 1: Valor por Status | Método de Pagamento ───────────────────────────
g1, g2 = st.columns(2)
...
with g1:
    if has_data:
        ...
        ax.set_facecolor("#f3f0fb")
        for sp in ax.spines.values():
            sp.set_edgecolor(BORDER_COLOR); sp.set_linewidth(1.8)

        bars = ax.barh(
            df_s["Status"], df_s["Valor"],
            color=[CORES_STATUS.get(s, "#7c3aed") for s in df_s["Status"]],
            edgecolor=BORDER_COLOR, linewidth=1.4,
        )
        for bar in bars:
            w = bar.get_width()
        ...
        img_html = _fig_to_html(fig)
        st.markdown(_card("💰 Valor por Status", img_html), unsafe_allow_html=True)
    else:
        st.info("Sem dados de pagamentos para este período.")
...
with g2:
    if has_data:
        ...
        wedges, texts, autotexts = ax2.pie(
            ...
            pctdistance=0.76,
            wedgeprops=dict(edgecolor=BORDER_COLOR, linewidth=2.5),
        )
        for t in autotexts:
            t.set_fontsize(9); t.set_fontweight("bold"); t.set_color("#1a0a2e")
        for t in texts:
            t.set_fontsize(9); t.set_color("#374151")
        ax2.set_title(f"Total: {int(df_m['Qtd'].sum())} cobranças",
                      fontsize=10, color="#374151", pad=8)
        plt.tight_layout()

        st.markdown(_card("💳 Método de Pagamento", _fig_to_html(fig2)), unsafe_allow_html=True)
    else:
        st.info("Sem dados de pagamentos para este período.")

_sep()

# ── Linha 2: Tendência Diária | Quantidade por Status ─────────────────────────
...
with g3:
    if has_data and not df_pay["dateCreated"].isna().all():
        ...
        st.markdown(_card("📈 Tendência Diária (R$)", _fig_to_html(fig3)), unsafe_allow_html=True)
    else:
...
with g4:
    if has_data:
        ...
        fig4, ax4 = plt.subplots(figsize=(4.5, 3.6))
        fig4.patch.set_facecolor(CARD_BG)
        ax4.set_facecolor("#f3f0fb")
        for sp in ax4.spines.values():
            sp.set_edgecolor(BORDER_COLOR); sp.set_linewidth(1.8)

        sns.barplot(
            data=df_cnt, y="Status", x="Qtd",
            palette=[CORES_STATUS.get(s, "#7c3aed") for s in df_cnt["Status"]],
            ax=ax4, edgecolor=BORDER_COLOR, linewidth=1.4,
        )
        ...
        st.markdown(_card("🔢 Quantidade por Status", _fig_to_html(fig4)), unsafe_allow_html=True)
    else:
        st.info("Sem dados.")

_sep()

# ── Linha 3: Status Convites | Parceiros por Perfil ───────────────────────────
g5, g6 = st.columns(2)
...
with g5:
    ...
    fig5.patch.set_facecolor(CARD_BG)
    ax5.set_facecolor("#f3f0fb")
    for sp in ax5.spines.values():
        sp.set_edgecolor(BORDER_COLOR); sp.set_linewidth(1.8)

    bars5 = ax5.bar(
        df_inv_chart["Status"], df_inv_chart["Qtd"],
        color=["#f59e0b", "#10b981", "#7c3aed"],
        edgecolor=BORDER_COLOR, linewidth=1.8, width=0.52,
    )
    ax5.bar_label(bars5, fmt="%d", padding=4, fontsize=11,
                  color="#374151", fontweight="bold")
    ...
    ax5.set_ylim(0, max(int(df_inv_chart["Qtd"].max()) * 1.35, 5))
    plt.tight_layout()

    st.markdown(_card("📨 Status dos Convites", _fig_to_html(fig5)), unsafe_allow_html=True)
...
with g6:
    if partners:
        ...
        labels_pt = {
            "b2b": "Profissional", "b2c": "Cliente",
            "admin": "Admin", "super_admin": "Super Admin", "demo": "Demo",
        }
        ...
        fig6.patch.set_facecolor(CARD_BG)
        ax6.set_facecolor("#f3f0fb")
        ...
        ax6.set_ylim(0, max(int(df_perf["Qtd"].max()) * 1.35, 5))
        plt.tight_layout()

        st.markdown(_card("👥 Parceiros por Perfil", _fig_to_html(fig6)), unsafe_allow_html=True)
    else:
    ...
    if not payments:
        if session_orders:
            ...
            for o in session_orders:
                ...
                st_val  = _status_badge(str(o.get("status", "")))
                brl_val = _brl(totais.get("total", 0))
                st.markdown(
                    f'<div style="background:#f8f7fc;border:2px solid #e5e0f5;border-radius:12px;'
                    f'padding:14px 18px;margin:8px 0;display:flex;justify-content:space-between;align-items:center;">'
                    f'<div><div style="font-size:.85rem;font-weight:700;color:#1a0a2e;">{ref_val}</div>'
                    f'<div style="font-size:.75rem;color:#6b7280;margin-top:2px;">{st_val}</div></div>'
                    f'<div style="font-size:1rem;font-weight:800;color:#7c3aed;">{brl_val}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
        else:
        ...
        for p in payments:
            ...
            st_txt    = _status_badge(str(p.get("status", "")))
            method_tx = _method_badge(str(p.get("billingType", "")))
            date_txt  = str(p.get("dateCreated") or "")[:10]
            brl_val   = _brl(p.get("value", 0))
            link_html = (f'<a href="{link}" target="_blank" '
                         f'style="font-size:.72rem;color:#7c3aed;">Ver cobrança</a>') if link else ""
            st.markdown(
                f'<div style="background:#f8f7fc;border:2px solid #e5e0f5;'
                f'border-left:5px solid {sc};border-radius:12px;'
                f'padding:14px 18px;margin:8px 0;display:flex;'
                f'justify-content:space-between;align-items:center;">'
                f'<div><div style="font-size:.85rem;font-weight:700;color:#1a0a2e;">{ref}</div>'
                f'<div style="font-size:.75rem;color:#6b7280;margin-top:2px;">'
                f'{st_txt} &nbsp;&middot;&nbsp; {method_tx} &nbsp;&middot;&nbsp; {date_txt}'
                f'</div></div>'
...
with tab_inv:
    ...
    if not invites:
        ...
        for inv in invites:
            ...
            email_i  = str(inv.get("email", ""))
            st.markdown(
                f'<div style="background:{bg_color};border:2px solid {bd_color};'
                f'border-radius:12px;padding:12px 18px;margin:6px 0;'
                f'display:flex;justify-content:space-between;">'
                f'<div><div style="font-size:.85rem;font-weight:700;color:#1a0a2e;">{email_i}</div>'
                f'<div style="font-size:.75rem;color:#6b7280;margin-top:2px;">'
                f'{role_lbl} &nbsp;&middot;&nbsp; Expira: {expires}</div></div>'
                f'<div style="font-size:.82rem;font-weight:700;color:{st_color};">{st_label}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
...
with tab_par:
    ...
    if not partners:
        ...
        icons_map = {"super_admin": "⭐", "admin": "🔑", "b2b": "💼", "b2c": "👤", "demo": "🧪"}
        for p in partners:
            ...
            st.markdown(
                ...
                f'<div><div style="font-size:.88rem;font-weight:700;color:#1a0a2e;">{nome_p}</div>'
                f'<div style="font-size:.75rem;color:#6b7280;">'
                f'{email_p} &nbsp;&middot;&nbsp; {loc}</div></div>'
                f'</div>',
                unsafe_allow_html=True,
            )
...
with tab_sess:
    ...
    if not session_orders:
        components.empty_state(icon="Sessão", title="Nenhum pedido na sessão",
        ...
        for o in session_orders:
            ...
            st.markdown(
                f'<div style="background:#f3f0ff;border:2px solid #c4b5fd;'
                f'border-radius:12px;padding:14px 18px;margin:8px 0;">'
                f'<div style="display:flex;justify-content:space-between;margin-bottom:6px;">'
                f'<span style="font-size:.85rem;font-weight:700;color:#1a0a2e;">{ext_ref}</span>'
                f'<span style="font-size:1rem;font-weight:800;color:#7c3aed;">{brl_val}</span>'
                f'</div>'
                f'<div style="font-size:.75rem;color:#6b7280;">'
                f'{status_lbl} &nbsp;&middot;&nbsp; ID: <code>{pay_id}</code>'
                f'</div></div>',
                unsafe_allow_html=True,
            )
        m1, m2 = st.columns(2)
