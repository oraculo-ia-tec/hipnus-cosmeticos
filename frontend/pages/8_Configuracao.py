...
v4 — 2026-07-01:
  - fix: avatar do usuário salvo com data-URI completo (data:mime;base64,...)
    para exibição correta no <img src> — antes era base64 puro sem prefixo.
  - fix: session_state["avatar_b64"] atualizado com data-URI completo.
"""
...
def _tab_minha_conta():
    ...
    if foto_up:
        ...
        if novo_hash != st.session_state.get("_avatar_upload_hash"):
            ...
            b64_raw   = base64.b64encode(foto_bytes).decode()
            # ✔ data-URI completo — necessário para <img src> funcionar
            data_uri  = f"data:{mime};base64,{b64_raw}"
            if DB_OK and email:
                # Persiste o data-URI completo no banco — não perde entre sessões
                atualizar_perfil(email=email, avatar_b64=data_uri)
            st.session_state["avatar_b64"]          = data_uri
    ...
    with col_up:
        ...
        if foto_upload:
            ...
            if novo_hash != st.session_state.get("chiara_foto_hash"):
                ...
                b64_raw = base64.b64encode(foto_bytes).decode()
                if DB_OK:
                    # Persiste base64 puro no banco (chiara usa mime separado)
                    salvar_foto_chiara(b64_raw, mime)
                st.session_state["chiara_foto_b64"]  = b64_raw
...
_FONTES_CORPO = ["Inter", "Poppins", "Nunito", "Lato", "Raleway", "DM Sans"]
_FONTES_TITULO = ["Syne", "Playfair Display", "Montserrat", "Outfit", "Space Grotesk", "Cormorant Garamond"]
...
def _tab_tema():
    ...
    with col_cores:
        ...
        tema_escolhido = st.radio(
            "Cor de acento", opcoes_tema, index=idx_tema,
            key="radio_tema_acento", label_visibility="collapsed",
        )
    ...
    with col_fontes:
        ...
        fonte_corpo_escolhida = st.selectbox(
            "Fonte do corpo (textos, botões, menus)", _FONTES_CORPO, index=idx_corpo,
            key="sel_fonte_corpo", help="Aplicada em parágrafos, inputs e botões.",
        )
        fonte_titulo_escolhida = st.selectbox(
            "Fonte dos títulos (H1, H2, H3)", _FONTES_TITULO, index=idx_titulo,
            key="sel_fonte_titulo", help="Aplicada em cabeçalhos, logo e seções.",
        )
        ...
        <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);
          ...
          <div style="font-family:'{fonte_titulo_escolhida}',serif;font-size:1.3rem;font-weight:800;
    ...
    <div style="background:{cores['sidebar_bg']};border:1px solid {cores['accent']}44;
      border-radius:16px;padding:20px;max-width:300px;
      box-shadow:0 0 32px {cores['glow']};font-family:'{fonte_corpo_escolhida}','Inter',sans-serif;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
        <div style="width:36px;height:36px;border-radius:10px;
          background:linear-gradient(135deg,{cores['accent']},{cores['accent_dark']});
          display:flex;align-items:center;justify-content:center;
          font-family:'{fonte_titulo_escolhida}',sans-serif;font-weight:800;font-size:1rem;color:#fff;
          box-shadow:0 0 14px {cores['glow']};">H</div>
        <div>
      ...
      <div style="display:flex;flex-direction:column;gap:3px;">
        <div style="padding:8px 12px;border-radius:9px;
          background:linear-gradient(135deg,{cores['accent']}33,{cores['accent_light']}14);
          border:1px solid {cores['accent']}44;color:#fff;font-size:.82rem;font-weight:600;">
          🤖  IA Consultora</div>
      ...
      <div style="padding:9px 14px;border-radius:11px;text-align:center;
        background:linear-gradient(135deg,{cores['accent_dark']}55,{cores['accent']}22);
        border:1px solid {cores['accent']}55;color:{cores['accent_light']};
        font-size:.82rem;font-weight:600;letter-spacing:.4px;
        box-shadow:0 0 10px {cores['glow']};
        font-family:'{fonte_corpo_escolhida}',sans-serif;">⧡  Sair da plataforma</div>
    </div>
    """)
    ...
    with col_btn:
        if st.button("🚀 Aplicar tema agora", key="btn_salvar_tema",
            ...
            if DB_OK:
                try:
                    ...
                    st.success(
                        ...
                        f"**{fonte_corpo_escolhida}** / **{fonte_titulo_escolhida}** aplicado e salvo!"
                    )
