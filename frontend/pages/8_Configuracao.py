...
=========================================
Página de Configuração de conta — disponível para TODOS os perfis autenticados.

Permite:
  - Visualizar e editar dados de perfil (nome, usuário, empresa, cidade, bio)
  - Fazer upload / remover foto de perfil circular
  - Alterar senha (apenas parceiros com conta no banco)
...
with tab_foto:
    ...
    with col_up:
        ...
        st.caption("📸 JPG, PNG ou WEBP · Máx 2 MB · Aparece no sidebar e futuramente no chat da 🤖 IA Consultora")
        ...
        if novo_avatar:
            if novo_avatar.size > 2 * 1024 * 1024:
                ...
                if st.button("💾 Salvar foto", type="primary", use_container_width=True):
                    if parceiro_db:
                        ok_s, _ = atualizar_perfil(email_logado, avatar_b64=novo_b64)
                    else:
                        ok_s = True
                    st.session_state["avatar_b64"] = novo_b64
                    st.success("✅ Foto atualizada!") if ok_s else st.warning("⚠️ Salvo só na sessão.")
                    st.rerun()
    if avatar_atual:
        components.divider()
...
with tab_senha:
    ...
    if not parceiro_db:
        st.info("🔑 Usuários seed/demo não podem alterar senha aqui. Edite diretamente o `auth.py`.")
    else:
