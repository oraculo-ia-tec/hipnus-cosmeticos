...
if submitted:
    if not email_dest:
        ...
        try:
            r = httpx.post(f"{API_V1}/invites/", json={"email": email_dest, "role": role_dest}, headers=headers, timeout=8.0)
            if r.status_code == 201:
                data = r.json()
                st.success(f"✅ Convite gerado para **{email_dest}**!\n\n**Token:** `{data.get('token', 'N/A')}`\n\n**Link:** `{data.get('signup_url', 'N/A')}`")
            else:
...
try:
    ...
    if r.status_code == 200:
        invites = r.json()
        if not invites:
            components.empty_state(icon="📨", title="Nenhum convite gerado", message="Gere o primeiro convite acima.")
        else:
            for inv in invites:
                status = "✅ Usado" if inv.get("used") else "⏳ Pendente"
                with st.expander(f"{inv.get('email', 'N/A')} — {status}"):
                    st.markdown(f"**Perfil:** {inv.get('role')}")
                    st.markdown(f"**Token:** `{inv.get('token')}`")
                    st.markdown(f"**Criado em:** {inv.get('created_at')}")
