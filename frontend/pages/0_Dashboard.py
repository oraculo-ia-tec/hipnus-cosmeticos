"""
0_Dashboard.py — HIPNUS COSMÉTICOS
=======================================
...
st.set_page_config(page_title="Dashboard - HIPNUS", page_icon="\U0001f4ca", layout="wide")
ui.inject_theme()
...
components.page_header(
    ...
    subtitle="Visao consolidada de cobracas, graficos Seaborn, convites e parceiros da plataforma.",
...
def _resolve_api_key() -> tuple[str, str]:
    ...
    try:
        ...
        keys_found = []
    motivo = (
        f"ASAAS_API_KEY nao encontrada. "
        f"Chaves em st.secrets: {keys_found}"
    )
    return "", motivo
...
if cache_key not in st.session_state:
    with st.spinner("Buscando cobrancas na API Asaas..."):
...
3. Confirme que o secret esta assim (na raiz, sem secao):
```toml
ASSAS_API_KEY = "$aact_SUA_CHAVE_AQUI"
```
> Se preferir usar secao:
```toml
[asaas]
ASSAS_API_KEY = "$aact_SUA_CHAVE_AQUI"
```
...
k1.metric("Recebido",       _brl(total_recebido), f"{n_recebido} cobrancas")
k2.metric("Pendente",       _brl(total_pendente), f"{n_pendente} cobrancas")
k3.metric("Vencido",        _brl(total_vencido),  f"{n_vencido} cobrancas")
k4.metric("Total cobrancas", len(payments) or len(session_orders))
...
with g_col1:
    ...
    if has_data:
        ...
        ax.set_title(f"Ultimos {dias} dias", fontsize=10, color="#374151", pad=8)
...
with g_col2:
    ...
    if has_data:
        ...
        for t in texts:
            t.set_fontsize(9)
            t.set_color("#374151")
        ax2.set_title(f"Total: {df_m['Qtd'].sum()} cobrancas", fontsize=10, color="#374151", pad=8)
        plt.tight_layout()
...
tab_cob, tab_inv, tab_par, tab_sess = st.tabs(
    ["Cobrancas", "Convites", "Parceiros", "Sessao"])

with tab_cob:
    components.section_title(f"Cobrancas — ultimos {dias} dias")
    if not payments:
        if session_orders:
            ...
            components.empty_state(icon="", title="Sem cobrancas",
        ...
        for p in payments:
            ...
            <div style="background:#f8f7fc;border:2px solid #e5e0f5;
              ...
              <div style="text-align:right;">
                ...
                {f'<a href="{link}" target="_blank" style="font-size:.72rem;color:#7c3aed;">Ver cobranca</a>' if link else ''}
              </div>
            </div>""")
...
with tab_inv:
    ...
    if not invites:
        ...
        for inv in invites:
            ...
            role_lbl = {"b2b": "Profissional", "b2c": "Cliente", "admin": "Admin"}.get(
            ...
            <div style="background:{'#f0fdf4' if usado else '#faf7ff'};
              ...
              <div style="font-size:.82rem;font-weight:700;
                color:{'#16a34a' if usado else '#7c3aed'};">
                {'Usado' if usado else 'Pendente'}
              </div>
            </div>""")
...
with tab_par:
    ...
    if not partners:
        ...
        icons_map = {"super_admin": "S", "admin": "A", "b2b": "B2B", "b2c": "B2C", "demo": "D"}
        for p in partners:
            perfil = p.get("perfil") or p.get("role", "")
            icon   = icons_map.get(perfil, "P")
            ...
            <div style="background:#f8f7fc;border:2px solid #e5e0f5;
              ...
              <div style="font-size:1rem;font-weight:700;color:#7c3aed;">[{icon}]</div>
              <div>
...
with tab_sess:
    ...
    if not session_orders:
        components.empty_state(icon="", title="Nenhum pedido na sessao",
        ...
        for o in session_orders:
            ...
            <div style="background:#f3f0ff;border:2px solid #c4b5fd;
              ...
              <div style="font-size:.75rem;color:#6b7280;">
                {_status_badge(o.get('status', ''))} · ID: {o.get('payment_id', '-')}
              </div>
            </div>""")
        m1, m2 = st.columns(2)
